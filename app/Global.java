	...
	public void onStart(Application app) {
		if (User.check("suad@suad.com") == false) {
			...
			if (User.check("haris.krkalic@bitcamp.ba") == false) {
				User testUser = User.createUserReturnUser(
						"haris.krkalic@bitcamp.ba", "Banana88");
		...
		User.createRestaurant("Lovac", "restoran@njam.ba", "123456",
				"08:00h - 18:00h", "Sarajevo", "Fojnicka", "5");
		User.createRestaurant("Harambasa", "restoran1@njam.ba", "123456",
				"07:00h - 19:00h", "Banjaluka", "Pere Kvrzice", "18");
		User.createRestaurant("Klopa", "restoran2@njam.ba", "123456",
				"09:00h - 20:00h", "Tuzla", "Marsala Tita", "22");

		Restaurant res3 = User.createRestaurantReturnRestaurant("Obala",
				"restoran3@njam.ba", "123456", "09:00h - 20:00h", "Sarajevo",
				"Jufkica 12", "21");
		Restaurant res4 = User.createRestaurantReturnRestaurant("Trovač",
				"restoran4@njam.ba", "123456", "09:00h - 20:00h", "Sarajevo",
				"Bananica 122", "27");
		...
		for (int i = 1; i <= 8; i++) {
			if (Meal.check(i) == false) {
				Meal.create("Pizza", 5.00, "Pizza", res1,
						"Povrce, gljive, sos, sir.");
				Meal.create("Pizza vegetariana", 5.00, "Pizza", res1,
						"Povrce, gljive, sos, sir.");
				Meal.create("Supa", 3.00, "Bosnian cousine", res1,
						"Sjeckano povrce sa piletionm.");
				Meal.create("Cevapi", 5.00, "Bosnian cousine", res1,
						"Cevapi u lepini sa lukom.");
				Meal.create("Pjeskavica", 6.00, "Fast Food", res1,
						"Pljeskavica, pomfrit, salata, majoneza, ketchup.");
		...
		if (Faq.check("Kako ću znati da li je restoran primio moju narudžbu? ") == false) {
			Faq.create(
					"Kako ću znati da li je restoran primio moju narudžbu? ",
